...
sealed trait List[+A] {
  ...
  def zipWith[B >: A](that: List[B])(f: (B, B) => B): List[B] =
    (this, that) match {
      case (_, Nil)                     => Nil
      case (Nil, _)                     => Nil
  ...
  def hasSubsequence[B >: A](sub: List[B]): Boolean =
    (this, sub) match {
      case (Cons(ha, ta), Cons(hb, tb)) if ha == hb => ta.hasSubsequence(tb)
      case (Cons(_, ta), _)                         => ta.hasSubsequence(sub)
      case (_, Nil)                                 => true
      case _                                        => false
    }
}
...
object List {
  ...
  def addingElements(xsx: List[Int], xsy: List[Int]): List[Int] =
    xsx match {
      case Nil => Nil
      case Cons(xHead, xTail) =>
        xsy match {
          case Nil                => Nil
  ...
  def double2String(nums: List[Double]): List[String] =
    nums match {
  ...
  def productFoldRight(xs: List[Double]): Double =
    xs.foldRight(1.0) { (elem, init) =>
      if (elem == 0.0) 0.0 else elem * init
    }

  def productFoldLeft(xs: List[Double]): Double =
    xs.foldLeft(1.0) { (init, elem) =>
      if (elem == 0.0) 0.0 else elem * init
    }
}
